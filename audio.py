...
async def help(ctx):
    ...
    embed.add_field(name=":warning: __Moderations Commands:__", value="``kick`` ``ban`` ``mute`` ``unmute`` ``clear`` ``say`` ``dm`` ``unban`` ``setupwelcomer`` ``setuplog`` ``announce`` ``embed``",inline = False)
    embed.add_field(name=":clapper: __Action Commands:__", value="``poke`` ``kiss`` ``slap`` ``hug`` ``bite`` ``pat`` ``bloodsuck`` ``cuddle`` ``thuglife`` ``burned`` ``savage`` ``facedesk`` ``highfive``",inline = False)		      
    embed.add_field(name=":earth_asia: __General Commands:__", value="``ping`` ``info`` ``serverinfo`` ``membercount`` ``gulidicon`` ``guildcount`` ``online`` ``offline`` ``stats`` ``joined`` ``botinfo``",inline = False) 		
    embed.add_field(name=":star2: __Fun Commands:__", value="``kiss`` ``meme`` ``slap`` ``hug`` ``joke`` ``movie`` ``tweet`` ``happybirthday`` ``flipcoin`` ``rolldice`` ``coinflip`` ``dice``",inline = False)	
    embed.add_field(name=":frame_photo: __Image Commands:__", value="``meme`` ``dog`` ``fox`` ``cat`` ``img`` ``randomshow`` ``neko`` ``buddy``",inline = False)	
    embed.add_field(name=":gear: __Misc Commands:__", value="``tweet`` ``trans`` ``eightball``",inline = False)
    embed.add_field(name=":clipboard: __Informational Commands:__", value="``movie`` ``mal`` ``randomshow``",inline = False)
    embed.add_field(name=":bulb: __Credits__", value="**Credits to the following:**",inline = False)
    embed.add_field(name=":bust_in_silhouette: __Head CEO of Buddy. Main Bot Developer.__", value="`Pratyaksh Dandyan#9700`",inline = False)
    embed.add_field(name=":busts_in_silhouette: __Main Bot Helper__", value="`☠The Invisible Imran☠#4615`",inline = False)
